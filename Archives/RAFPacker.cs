...
namespace Dargon.IO.RADS.Archives {
   public class RAFPacker {
      ...
      private List<string> uncompressedFiles = null;
      private List<string> archivedPaths = null;
      ...
      public bool PackRAF(string sourceDirectory, string targetDirectory) {
         ...
         for (int i = 0; i < archivedPaths.Count; i++) {
            ...
            if (uncompressedFiles.Contains(archivedPaths[i].ToLower())) {
               ...
               var deflate = new DeflateStream(new MemoryStream(fileContent), CompressionLevel.Optimal);
               deflate.CopyTo(mStream);
